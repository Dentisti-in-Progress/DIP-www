...
const site: SiteEditorial = { id: "reef", depot: "owner/reef", branche: "test/branch", dossierArticles: "src/data/posts", dossierAuteurs: "src/data/authors", dossierSujets: "src/data/topics", dossierImages: "src/assets/covers", format: "reef", langues: ["it", "en"] };
...
const request = (body: unknown, origin = "https://reef.test") => new Request("https://reef.test/api/editorial-categories/it/design", { method: "POST", headers: { Origin: origin, "Content-Type": "application/json" }, body: JSON.stringify(body) });
const valid = { sha: "original", frontmatter: { name: "Création", description: "Rubrique française", accent: "coral", order: 2 } };
role = "anonymous"; assert.equal((await editor(request(valid), "it", "design")).status, 401);
role = "customer"; assert.equal((await editor(request(valid), "it", "design")).status, 403);
role = "admin"; assert.equal((await editor(request(valid, "https://other.test"), "it", "design")).status, 403);
assert.equal(calls.length, 0);
assert.equal((await editor(request(valid), "it", "../design")).status, 400);
assert.equal((await editor(request({ ...valid, sha: "stale" }), "it", "design")).status, 409);
assert.equal((await editor(request({ ...valid, frontmatter: { ...valid.frontmatter, order: 2.4 } }), "it", "design")).status, 422);
assert.equal(calls.filter(call => call.path.endsWith("/git/trees")).length, 0);
assert.equal((await editor(request(valid), "it", "design")).status, 200);
...
assert.equal((await editor(request({ action: "delete", sha: "original" }), "it", "design")).status, 422);
assert.equal(calls.filter(call => call.path.endsWith("/git/trees")).length, count);
used = false; concurrent = true;
assert.equal((await editor(request({ action: "delete", sha: "original" }), "it", "design")).status, 409);
concurrent = false;
assert.equal((await editor(request({ action: "delete", sha: "original" }), "it", "design")).status, 200);
